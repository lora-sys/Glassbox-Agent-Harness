...
P5B has one owner, one implementation branch and one later implementation PR. It is intentionally parallel with P5A and starts from the same post-P4 main baseline.
...
P5B can develop with fixture ContextDemandEstimate values until P5A lands.

## Parallel worktree boundary

Preferred P5B ownership:

~~~text
packages/contracts/src/runtime-observability.ts
...
Core routing/collector logic must live in leaf modules before these hotspots are edited. Integration commits are serialized/rebased with P5A rather than developed as competing copies.
...
The one P5B implementation PR must list:
