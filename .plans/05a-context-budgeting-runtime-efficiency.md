...
P5A production ownership in the combined PR:

~~~text
apps/server/src/efficiency/**
apps/server/src/runtime/pi/run-adapter.ts
apps/server/src/execution/model-adapter.ts
focused P5A tests
~~~
...
These are shared integration hotspots. The combined PR integrates them after the leaf modules.
...
The combined P5 implementation PR must list for every substantially ported mechanism:
