...
`getGroupMemberRole` is the narrow execution-time role read. It calls `get_group_member_info` over the authenticated socket with `no_cache=true`, validates that the response names the requested configured group and sender, and returns only the normalized role. Every group mutation enabled by a QQ-native role performs this read after Glassbox policy, Resource authorization, and current-message intent checks, but before the provider mutation. A demotion therefore blocks the pending mutation. A disconnected role lookup is provider unavailable, not a caller-role denial. Raw member profiles never leave the adapter.
