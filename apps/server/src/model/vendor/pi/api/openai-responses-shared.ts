...
export async function processResponsesStream<TApi extends Api>(
  ...
  const finalizeResponse = (
    ...
    if (response?.usage) {
      ...
      output.usage = {
        reported: {
          input: typeof response.usage.input_tokens === "number",
          output: typeof response.usage.output_tokens === "number",
          cacheRead: inputDetails?.cached_tokens != null,
          cacheWrite: inputDetails?.cache_write_tokens != null,
          reasoning: response.usage.output_tokens_details?.reasoning_tokens != null,
          totalTokens: typeof response.usage.total_tokens === "number",
        },
    ...
    if (event.type === "response.created") {
      ...
      pushToolCallDelta(
        ...
        appendCustomToolCallInput(
          slot.block,
          getCustomToolCallInput(slot.block) + event.delta,
          false,
        ),
      );
      ...
      if (item.type === "reasoning" && slot?.type === "thinking") {
        ...
        pushToolCallDelta(
          ...
          appendCustomToolCallInput(
            slot.block,
            item.input ?? getCustomToolCallInput(slot.block),
            true,
          ),
        );
        if (item.namespace !== undefined) slot.block.namespace = item.namespace;
