...
function getAnthropicCompat(model: Model<"anthropic-messages">) {
  ...
  return {
    ...
    sessionAffinityFormat:
      model.compat?.sessionAffinityFormat ?? (isOpenRouter ? "openrouter" : undefined),
...
export const stream: StreamFunction<"anthropic-messages", AnthropicOptions> = (
  ...
  void (async () => {
    const providerThinkingLevel = model.compat?.supportsMidConvoEffort
      ? (options?.effort ?? "high")
      : undefined;
      ...
      for await (const event of iterateAnthropicEvents(response, options?.signal)) {
        if (event.type === "message_start") {
          ...
          output.usage.reported = {
            input: typeof event.message.usage.input_tokens === "number",
            output: typeof event.message.usage.output_tokens === "number",
          ...
          if (event.usage) {
            output.usage.reported = {
              ...output.usage.reported,
              ...(event.usage.input_tokens != null && { input: true }),
              ...(event.usage.output_tokens != null && { output: true }),
              ...(event.usage.cache_read_input_tokens != null && { cacheRead: true }),
              ...(event.usage.cache_creation_input_tokens != null && { cacheWrite: true }),
              ...(event.usage.output_tokens_details?.thinking_tokens != null && {
                reasoning: true,
              }),
            };
...
function createClient(
  ...
  _dynamicHeaders?: Record<string, string>,
  sessionId?: string,
): { client: Anthropic; isOAuthToken: boolean } {
  // Glassbox supports explicit API credentials only.
