...
/** Explicit provider-scoped environment values. No host environment fallback. */
...
export interface ProviderRequestOptions<TModel = Model<Api>> {
  ...
  fetch?: FetchFunction;
  /**
   * Explicit provider-scoped environment values for
...
export interface Usage {
  /** Glassbox addition: fields supported by actual provider usage evidence. */
  reported?: Partial<
    Record<"input" | "output" | "cacheRead" | "cacheWrite" | "reasoning" | "totalTokens", boolean>
  >;
  input: number;
  output: number;
  cacheRead: number;
...
export interface ImagesModel<TApi extends ImagesApi> extends Omit<
  Model<Api>,
