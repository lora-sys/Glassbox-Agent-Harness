...
async function verifySandboxLock(kitPath: string, image: string): Promise<void> {
  ...
  const required = [
    ...
    "dist/src/sandbox/egress-proxy.js",
  ];
...
export async function loadKitSandbox(kitPath: string): Promise<{
  ...
  const dnsMode = process.env.GLASSBOX_SANDBOX_DNS_MODE ?? "system";
  if (dnsMode !== "system" && dnsMode !== "cloudflare_doh")
    throw new Error("GLASSBOX_SANDBOX_DNS_MODE must be system or cloudflare_doh");
  const module = (await import(pathToFileURL(kitModulePath).href)) as {
    createDockerSandboxExecutor?: (input: {
      image: string;
  ...
  const executor = module.createDockerSandboxExecutor({ image, dnsMode });
  ...
  try {
    ...
    return {
      executor,
      availableTools: new Set(probe.availableTools),
