...
export function localDatabaseUrl(databasePath: string): string {
  ...
  if (!databasePath || /^(?:[a-z][a-z\d+.-]*:)/iu.test(databasePath.replace(/^[a-z]:[\\/]/iu, '')) || /^(?:\\\\|\/\/)/u.test(databasePath)) {
...
export class DomainDatabase {
  ...
  static async open(databasePath: string): Promise<DomainDatabase> {
    ...
    if (databasePath !== ':memory:') await mkdir(dirname(resolve(databasePath)), { recursive: true });
    ...
    try {
      ...
      await db.transaction(async (tx) => {
        const version = Number((await tx.execute('PRAGMA user_version')).rows[0]?.user_version ?? 0);
