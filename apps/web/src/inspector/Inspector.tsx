...
export type ObjectMeta = {
  ...
  | {
      ...
      _onApprove: (itemId: string) => void;
      _onDecline: (itemId: string) => void;
    }
);
...
function TraceLine({ entry }: { entry: TraceEntryLike }) {
  ...
  } else if (typeof p.task === "string") {
    detail = `"${p.task.slice(0, 42)}"`;
  } else if (typeof p.instruction === "string") {
    detail = `"${p.instruction.slice(0, 42)}"`;
  ...
  return (
    ...
    >
      ...
      <span style={{ color: accent, fontFamily: "monospace" }}>{method}</span>
      {detail && <div style={{ color: textMuted, padding: "1px 0 0 20px" }}>{detail}</div>}
    </div>
  );
}
...
function matchesObject(e: TraceEntryLike, om: ObjectMeta | null): boolean {
  if (!om) return true;
  ...
  switch (om.objectType) {
    ...
    case "work":
      return (
        ["item/started", "item/agentMessage/delta"].includes(m ?? "") && p.itemId === om.itemId
      );
    case "artifact":
      return m === "item/fileChange" && p.itemId === om.itemId;
    case "testResult":
      return m === "item/completed" && (p.item as { id?: string } | undefined)?.id === om.itemId;
    case "finalResult":
    ...
    case "steer":
      return (
        (m === "action.steer" || m === "action.stop" || m === "action.send") &&
...
function ObjectFields({
      ...
      <Section label={isDrafting ? "Prompt (editing)" : "Prompt"}>
        <textarea
          ...
          style={{
            ...
            border: isDrafting ? `1px solid ${draftColor}` : "none",
            borderRadius: 4,
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
              ...
              <button
                ...
                style={{
                  ...
                  background: isApplying ? "#1f2028" : "#22232e",
                  color: isApplying ? "#52525b" : "#a1a1aa",
                  fontSize: 11,
                  fontWeight: 600,
                  cursor: isApplying ? "not-allowed" : "pointer",
  ...
  if (selectedObject.objectType === "artifact") {
    const arts = ((derivedState as { artifacts?: unknown[] })?.artifacts ?? []) as {
      itemId: string;
  ...
  if (selectedObject.objectType === "testResult") {
    ...
    return (
      <Section label="Test Output">
        ...
        >
          <div>
            Status: <span style={{ color: textPrimary }}>{tr?.status ?? "—"}</span>
          </div>
          {tr?.exitCode != null && (
            <div>
              Exit: <span style={{ color: textPrimary }}>{String(tr.exitCode)}</span>
            </div>
          )}
          {tr?.durationMs != null && (
            <div>
              Duration: <span style={{ color: textPrimary }}>{tr.durationMs}ms</span>
            </div>
          )}
          {tr?.aggregatedOutput && (
            <pre
      ...
      <Section label="Result">
        ...
        >
          ...
          {fr?.durationMs != null && (
            <div>
              Duration:{" "}
              <span style={{ color: textPrimary }}>{(fr.durationMs / 1000).toFixed(1)}s</span>
            </div>
          )}
          {fr?.error && <div style={{ color: "#f87171" }}>Error: {fr.error.slice(0, 200)}</div>}
        </div>
      </Section>
    );
  }
  if (selectedObject.objectType === "turnResult") {
    ...
    if (!t)
      return (
        <Section label="Turn">
          <div style={{ fontSize: 11, color: muted }}>Turn data not found</div>
        </Section>
      );
    return (
      <Section label="Turn Details">
        ...
        <div style={{ fontSize: 11, lineHeight: 1.6 }}>
          <div>
            Input: <span style={{ color: textPrimary }}>{t.taskOrInstruction || "(empty)"}</span>
          </div>
          {t.finalResult && (
            <div>
              Status:{" "}
  ...
  if (selectedObject.objectType === "traceSummary") {
    ...
    if (!ts)
      return (
        <Section label="Trace">
          <div style={{ fontSize: 11, color: muted }}>No trace data</div>
        </Section>
      );
    return (
      ...
      <Section label="File-Change Decision">
        <div style={{ fontSize: 11, lineHeight: 1.6, marginBottom: 8 }}>
          <div>
            ...
            <span style={{ color: textPrimary, fontFamily: "monospace" }}>{itemId}</span>
          </div>
          <div>
            <span style={{ color: muted }}>Reason:</span>{" "}
            <span style={{ color: textPrimary }}>{reason}</span>
          </div>
          {grantRoot && (
            <div>
              <span style={{ color: muted }}>Scope:</span>{" "}
              <span style={{ color: textPrimary, fontFamily: "monospace" }}>{grantRoot}</span>
            </div>
          )}
        </div>
        ...
        <div style={{ display: "flex", gap: 8 }}>
          <button
            onClick={() => pending._onApprove(itemId)}
            style={{
              padding: "6px 18px",
              borderRadius: 4,
              border: "none",
          ...
          <button
            onClick={() => pending._onDecline(itemId)}
            style={{
              padding: "6px 18px",
              borderRadius: 4,
              border: "none",
  ...
  if (selectedObject.objectType === "systemInstruction") {
    const appliedInstruction =
      (derivedState as { systemInstruction?: string })?.systemInstruction ?? "";
...
export function Inspector({
  ...
  const filtered = (() => {
    ...
    return traceEntries.filter((e) => matchesObject(e, selectedObject)).reverse();
  })();
      ...
      <>
        ...
        <Section label="Totals">
          ...
          {ts.totalDurationMs != null && (
            <div style={{ fontSize: 11, color: textMuted }}>
              Duration:{" "}
              <span style={{ color: textPrimary }}>{(ts.totalDurationMs / 1000).toFixed(1)}s</span>
            </div>
          )}
        </Section>
      </>
    ...
    <div style={{ fontSize: 12, color: textMuted }}>
      ...
      >
        {hasSelection ? (
          ...
          <div style={{ fontSize: 13, color: muted }}>Select an object to inspect</div>
        )}
      </div>
      ...
      <Section label={`Trace (${filtered.length})`}>
        {filtered.length === 0 ? (
          <div style={{ fontSize: 11, color: dim, padding: "4px 0" }}>No matching trace events</div>
        ) : (
